...
def index():
    ...
    groupedstockquantity = db.execute("SELECT stock_symbol, SUM(quantity) AS quantity, SUM(total_amount_paid) AS total_cash_per_stock_symbol, stock_price FROM buystocks WHERE user_id = ? GROUP BY stock_symbol", user_id)
    ...
    for row in groupedstockquantity:
        ...
        print(f"{quantity}")
        total_cash_per_stock_symbol_dict[stock_symbol] = quantity * price
        stock_value_sum += total_cash_per_stock_symbol_dict[stock_symbol]
        print(f"{pricedict}")

    cash = db.execute("SELECT cash FROM users where id = ?", user_id)[0]["cash"]
    total_value = round(cash + stock_value_sum, 2)
    #print cash, print portfolio value
    ##variables: cash_value; total_value;
    return render_template("index.html", groupedstockquantity=groupedstockquantity, cash=cash, total_cash_per_stock_symbol_dict=total_cash_per_stock_symbol_dict, total_value=total_value, user = user)
...
def history():
    if request.method == "GET":
        user_id = session["user_id"]
        history_list_of_dict = db.execute("SELECT timestamp, stock_symbol, stock_price, total_amount_paid, quantity FROM buystocks WHERE user_id = ?", user_id)
...
def sell():
    ...
    if request.method == "GET":
        ...
        for dictofstocks in groupedstockquantity:
            if dictofstocks["stock_symbol"] == symbol:
                user_shares_quantity = dictofstocks["quantity"]
        ...
        if input_shares_quantity < 0:
            ...
            db.execute("INSERT INTO buystocks (user_id, timestamp, stock_symbol, stock_price, total_amount_paid, quantity) VALUES (?, ?, ?, ?, ?, ?)", user_id, timestamp, symbol, price, total_amount_paid, reflected_shares_quantity)
